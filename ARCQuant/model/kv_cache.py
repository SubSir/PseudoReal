...
class _AttentionStub(object):
    ...
    def forward(self, q, num_kv_heads, attention_kwargs, layer_idx):
        ...
        if self.hadamard_dtype is not None:
            q = matmul_hadagemm(q, dtype=self.hadamard_dtype) 
...
class MultiLayerPagedKVCache4Bit(Cache):
    ...
    def _ensure_page_cnt_per_batch(self, expected_page_cnt_per_batch):
        expected_total_pages = expected_page_cnt_per_batch * self.batch_size
        current_capacity = self.pages.shape[0]
        ...
        raise ValueError(
            f"KV Cache capacity exceeded. The cache was initialized with a max_seq_len "
            f"that supports approximately {current_max_len} tokens, but the generation "
            f"process now requires space for more tokens. Please initialize the cache "
    ...
    def update(
        ...
        if self.hadamard_dtype is not None:
            key_states = matmul_hadagemm(key_states, dtype=self.hadamard_dtype)
